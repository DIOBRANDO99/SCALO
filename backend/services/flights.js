import { getHubsWithDetails, findHubs } from "./hubs.js";
...
export async function discoverStopovers({ origin, destination, outboundDate, returnDate, stopoverNights }) {
    // Get route-verified hubs from the ellipse pipeline
    const hubData = getHubsWithDetails(origin, destination);
    const hubs = hubData ? hubData.hubs.map(h => h.iata) : FALLBACK_HUBS;
